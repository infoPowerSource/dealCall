			...
			success : function(data) {
				...
				$.each(data.partyInfo, function(index, party) {
					...
					$("#custList").append(
						...
						+((data.conference.confStatus != 1 && data.conference.confStatus != 4 && data.conference.confStatus != 5) || party.partyPhone =="" ? "<td><div class='m-txt'></div></td>" : "<td><a class='m-txt " + (callingStatus[party.partyStatus].en) +"'"+ (party.partyStatus ==1 ? " ontouchstart='calling(this)' href='#hujiaozhuanjia'" : " ")+">" + (callingStatus[party.partyStatus].zh) + "</a></td>")
						+"</tr>" 
					);
...
function changeFuction(){
	...
	if(confStatus==1 || confStatus==4){
		...
		+"</li>");		
	}else if(confStatus==0  || confStatus == 5){
		//会议未开始，可以编辑和删除会议
		$("#fuc_other li").remove();
		$("#fuc_other").append(
		"<li class='mui-table-view-cell'>"
