        ...
        if (confId) {
            ...
            $.getJSON(BASE_PATH + "/create/getConferenceData/" + confId, function(confData){
                if(confData){
                    ...
                    _this.form.anpai.cfg.notifyemail=confData.ifMail;
                    _this.form.anpai.cfg.notifysms=confData.ifSms;
                    _this.form.anpai.cfg.join=confData.ifLimitParty;
            ...
            $.getJSON(BASE_PATH + "/conf/last/conference", function(data){
                if(data.success == "true"){
                    _this.form.anpai.cfg.hjKey=data.confConfig;
                    _this.form.anpai.cfg.notifyemail=data.ifMail;
                    _this.form.anpai.cfg.notifysms=data.ifSms;
                }
            });
        }
    },
        ...
        inpclick:function(key,data,index){
            ...
            switch(objId){
             ...
             case "addConf":
                ...
                if(data.cfg.hjKey == "3"){
                    for(var i=0;i<data.kh.length;i++){
                        if(data.kh[i].dat.pho == "" && (data.kh[i].dat.nam != "" || data.kh[i].dat.ema != "")){
...
//        	}
    ...
    function hideSelectItem(data){
    	...
    	var length=0;
    	for(var i=0;i<len;i++){
    		if(_this.form.anpai.kh[i].dat.nam !=""){
