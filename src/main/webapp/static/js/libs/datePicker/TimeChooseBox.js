...
var TimeChooseBox = function (container,options) {
    if (!container) {
        return;
    }
    _this = this;
...
TimeChooseBox.prototype = {
    ...
    proccessKeypress: function (e, keyCode) {
        ...
        if (selectStart == 0 && selectEnd == 0) {
            ...
            var _one = parseInt(e.target.value.substring(0, 1));
        ...
        else if (selectStart == 4) {
            if (e.target.value.length == 5) {
                e.target.value = e.target.value.substr(0, e.target.value.length - 1);
            }
        }
        else {
            if ( selectEnd - selectStart >= 4) {
                return e.preventDefault();
            }
