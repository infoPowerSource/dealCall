...
public class ConfCdrServiceImpl implements IConfCdrService{
	...
	public List<ConfDetailCDR> getConfCDRToDeal(ConferenceInfo conf, ConferenceWaitMake conferenceWaitMake){
		...
		try{
			for(String msRecordId : confMap.keySet()){
				...
				// 通过计费接口获取会议明细信息，查询明细时间用查询会议的时间
				pre_confDetailCDRList = getConfDetailCDR(confCDR, conf.getConfId(), startTime, endTime);
	 ...
	 */
	public List<ConfDetailCDR> getConfDetailCDR(ConfCDR confCDR, Long confId, String startTime, String endTime){
	...
	public HashMap<String, ConfCDR> getConfCDR(String billingcode, String startTime, String endTime){
		...
		try{
			...
			for(int i = 0; i < conflist.length(); i++){
				...
				//搜索所有场次，只有当所有会议都结束且采集成功后，才能开始生成报告
				// 1：result非空 会议已结束并有记录 ;
				// 0：result为{} 会议已结束无会议记录;
				// -1：result为{} 会议未结束
				// -2：result为{} 无任何会议
				// -3：result为{} 会议结束是采集有问题
				// -4：result为{} 会议结束采集中
				if(null == confcdrobject 
					|| null == confcdrobject.getString("status") 
					|| "".equals(confcdrobject.getString("status"))
					|| "-1".equals(confcdrobject.getString("status"))
					|| "-3".equals(confcdrobject.getString("status"))
					|| "-4".equals(confcdrobject.getString("status"))) {
					logger.info("getConfCDR->confcdrobject 会议没结束或采集没记录，不能生成报告 ;billingcode=" + billingcode);
					return null;
				}
			}
