...
public class ConferenceServiceImpl implements IConferenceService{
	...
	private void putDataToConferenceInfo(ConferenceInfoForm confForm, ConferenceInfo confInfo, String billingCode, List<ConferencePasswordInfo> passwordList){
		...
		confInfo.setIfMail(Integer.parseInt(confForm.getNotifyemail()));
		confInfo.setIfSms(Integer.parseInt(confForm.getNotifysms()));
		confInfo.setIfLimitParty(Integer.parseInt(confForm.getJoin()));
	}
	...
	public void addAppConferenceInfo(String data, UserAllInfo userInfo, String confId) throws ValidationException{
		...
		confForm.setNotifyemail(cfgObj.getString("notifyemail"));
		confForm.setNotifysms(cfgObj.getString("notifysms"));
		confForm.setJoin(cfgObj.getString("join"));
	...
	public ConferenceInfo getLastConf(String billingCode){
		List<ConferenceInfo> confList = confDao.getLastConf(billingCode);
		if(confList.size()>0){
			ConferenceInfo confInfo =confList.get(0);
			return confInfo;
		}else{
			return null;
		}
