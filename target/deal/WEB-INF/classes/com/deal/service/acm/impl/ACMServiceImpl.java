...
public class ACMServiceImpl implements IACMService{
	...
	private String namePrefix="-DEAL-";
	...
	public int createConference(ConferenceInfo confInfo){
		...
		msgMap.put("ConferenceName",namePrefix+confInfo.getConfName());
		...
		// 参与人进入会议模式 M静音 ；T可听可讲
	    msgMap.put("GuestMode", "T");
		//根据billingcode，查询主账号同步属性，作为预约会议属性
		UserInfo useinfo = userInfoDao.getUserInfoByBillcode(confInfo.getAccountBillingcode());
		if(useinfo != null){
	...
	public int stopRecord(ConferenceInfo confInfo){
		String billingcode=confInfo.getConfBillingcode();
		if(!monitorService.isSubscribeAndActiveConf(billingcode)){
			logger.error("停止录音时会议订阅激活失败，BC:" + billingcode);
			return 1;
		}
		Conference conference = (Conference) confMgm.getConference(billingcode);
		if(ObjectUtils.isEmpty(conference)){
			logger.error("查询ACM会议为空，BC:" + billingcode);
			return 1;
		}
...
//		}
	...
	public int startRecord(ConferenceInfo confInfo){
		String billingcode=confInfo.getConfBillingcode();
		if(!monitorService.isSubscribeAndActiveConf(billingcode)){
			logger.error("开始录音时会议订阅激活失败，BC:" + billingcode);
			return 1;
		}
		Conference conference = (Conference) confMgm.getConference(billingcode);
		if(ObjectUtils.isEmpty(conference)){
