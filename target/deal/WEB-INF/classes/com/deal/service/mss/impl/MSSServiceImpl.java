...
public class MSSServiceImpl implements IMSSService{
	...
	public void sendMailByTask(Map<String, Object> params) throws ValidationException{
		String mailType = params.get("mailType").toString().trim();
		...
		if(template == null){
			...
			}else if(type==Consts.EMAIL_TYPE_FOR_INFORM_BC_WARNING||type==Consts.EMAIL_TYPE_FOR_INFORM_PC_WARNING){
				sendMailForWarning(params,template,type);
			}else{
	...
	private void sendMailForWarning(Map<String, Object> params, MSSTemplate template,int type){
		Map<String, String> dataMap = Maps.newHashMap();
		MSSSendTask mailTask = new MSSSendTask();
		String content=template.getTemplateContent();
		mailTask.setEmailContent(content);
		mailTask.setEmailReceiver(params.get("email").toString().trim());
		mailTask.setReceiveName(Consts.EMAIL_FOR_WARNING);
		mailTask.setEmailTitle(template.getEmailTitle());
		mailTask.setEmailType(type);
